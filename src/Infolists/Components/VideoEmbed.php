...
class VideoEmbed extends Entry
{
    ...
    public function getVideoEmbedHtml(string $url): string
    {
        if (str_contains($url, 'youtube.com') || str_contains($url, 'youtu.be')) {
            ...
            return '<div class="relative w-full -mx-6 -my-4" style="padding-bottom: 56.25%;"><iframe class="absolute top-0 left-0 w-full h-full rounded-lg" src="https://www.youtube.com/embed/' . $videoId . '" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe></div>';
        } elseif (str_contains($url, 'vimeo.com')) {
            $videoId = $this->getVimeoVideoId($url);
            $hash = $this->getVimeoHash($url);

            return '<div class="relative w-full -mx-6 -my-4" style="padding-bottom: 56.25%;"><iframe class="absolute top-0 left-0 w-full h-full rounded-lg" src="https://player.vimeo.com/video/' . $videoId . '?h=' . $hash . '&badge=0&autopause=0&player_id=0&app_id=58479" frameborder="0" allow="autoplay; fullscreen; picture-in-picture; clipboard-write; encrypted-media" allowfullscreen></iframe></div>';
        } elseif (str_contains($url, 'wistia.com')) {
            $videoId = $this->getWistiaVideoId($url);

            return '<div class="relative w-full -mx-6 -my-4" style="padding-bottom: 56.25%;"><iframe class="absolute top-0 left-0 w-full h-full rounded-lg" src="https://fast.wistia.net/embed/iframe/' . $videoId . '" frameborder="0" allow="autoplay; fullscreen" allowfullscreen></iframe></div>';
        }
